...
7. Tunnel setup (first time, or if `make dev-backend` fails): read [the local development instructions](./apps/api/README.md#local-development). `APP_DOMAIN` (api), `VITE_API_BASE_URL` (web), and `EXPO_PUBLIC_API_BASE_URL` (mobile) must all name the same tunnel domain or OAuth state cookies won't flow end-to-end. Payments and SSO providers (GitHub, Apple) additionally depend on it, since they reject `http://localhost` callbacks.
